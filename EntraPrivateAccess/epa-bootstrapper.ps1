...
#region "Join the AD Domain (if required)"
    ...
    if($needsAdJoin)
    {
        ...
        Start-Job { Start-Sleep 60; Restart-Computer -Force }
        exit 0
    }
...
#region "Download the connector installer from MS, then do a silent install"
    ...
    if (-not (Test-Path $regScript)) {
        ...
        if ($auth.SignerCertificate.Subject -notlike "CN=Microsoft Corporation,*") { throw "Installer signature not by Microsoft: $($auth.SignerCertificate.Subject)" } else { Write-Stamp 'Installer signed by MS' }
